...
case object PackBitsAccTLB extends Field[Option[TLBConfig]](None)
...
class L2MemHelperModule(outer: L2MemHelper, printInfo: String = "", queueRequests: Boolean = false, queueResponses: Boolean = false, printWriteBytes: Boolean = false)(implicit p: Parameters) extends LazyModuleImp(outer)
  ...
  when (io.status.valid) {
    PackBitsAccLogger.logInfo(printInfo + " setting status.dprv to: %x compare %x\n", io.status.bits.dprv, PRV.M.U)
    status := io.status.bits
  }

  val tlb = Module(new TLB(false, log2Ceil(coreDataBytes), p(PackBitsAccTLB).get)(edge, p))
  ...
  when (tags_init_reg =/= (outer.numOutstandingRequestsAllowed).U) {
    ...
    when (tags_for_issue_Q.io.enq.ready) {
      PackBitsAccLogger.logInfo(printInfo + " tags_for_issue_Q init with value %d\n", tags_for_issue_Q.io.enq.bits)
  ...
  when (!assertcheck) {
    PackBitsAccLogger.logInfo(printInfo + " L2IF: access addr must be aligned to write width\n")
  }
  assert(assertcheck,
  ...
  when (!assert_free_outstanding_op_slots) {
    PackBitsAccLogger.logInfo(printInfo + " L2IF: Too many outstanding requests for tag count.\n")
  }
  ...
  when (request_input.bits.cmd === M_XRD) {
    ...
    PackBitsAccLogger.logInfo(printInfo + " ERR")
    assert(false.B, "ERR")
  }
...
// val current_request_tag_has_response_space = tl_resp_queues(tags_for_issue_Q.io.deq.bits).enq.ready
  ...
  when (dmem.a.fire) {
    when (request_input.bits.cmd === M_XRD) {
      PackBitsAccLogger.logInfo(printInfo + " L2IF: req(read) vaddr: 0x%x, paddr: 0x%x, wid: 0x%x, opnum: %d, sendtag: %d\n",
    ...
    when (request_input.bits.cmd === M_XWR) {
      PackBitsAccLogger.logCritical(printInfo + " L2IF: req(write) vaddr: 0x%x, paddr: 0x%x, wid: 0x%x, data: 0x%x, opnum: %d, sendtag: %d\n",
        request_input.bits.addr,
        tlb.io.resp.paddr,
        request_input.bits.size,
      ...
      if (printWriteBytes) {
        for (i <- 0 until 32) {
          when (i.U < (1.U << request_input.bits.size)) {
            PackBitsAccLogger.logInfo("WRITE_BYTE ADDR: 0x%x BYTE: 0x%x " + printInfo + "\n", request_input.bits.addr + i.U, (request_input.bits.data >> (i*8).U)(7, 0))
          }
        }
      }
    }
...
// val selectQready = tl_resp_queues(dmem.d.bits.source).enq.ready
  ...
  when (fire_actual_mem_resp.fire(tags_for_issue_Q.io.enq.ready) &&
    tags_for_issue_Q.io.enq.valid) {
      PackBitsAccLogger.logInfo(printInfo + " tags_for_issue_Q add back tag %d\n", tags_for_issue_Q.io.enq.bits)
  }
...
// val resultdata = currentQueue.deq.bits.data >> (outstanding_req_addr.io.deq.bits.addrindex << 3)
  ...
  when (dmem.d.fire) {
    when (edge.hasData(dmem.d.bits)) {
      PackBitsAccLogger.logInfo(printInfo + " L2IF: resp(read) data: 0x%x, opnum: %d, gettag: %d\n",
        dmem.d.bits.data,
        global_memop_ackd,
        dmem.d.bits.source)
    } .otherwise {
      PackBitsAccLogger.logInfo(printInfo + " L2IF: resp(write) opnum: %d, gettag: %d\n",
        global_memop_ackd,
        dmem.d.bits.source)
    }
  }
  ...
  when (response_output.fire) {
    PackBitsAccLogger.logInfo(printInfo + " L2IF: realresp() data: 0x%x, opnum: %d, gettag: %d\n",
      resultdata,
